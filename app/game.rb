module Game
  ...
  def Game::renderables(game)
    [
        ...
        {x: 10, y: 90, text: "FPS : #{$gtk.current_framerate.to_s.to_i}", r: 255, g: 0, b:0},
        {x: 10, y: 60, text: "Bullet Count : #{game[:bullets].length}", r: 255, g: 0, b:0},
        {x: 10, y: 30, text: "Player Speed : #{(100.0*XYVector.abs(game[:player][:vel])/game[:player][:attrs][:physics][:base_speed]).round}%", r: 255, g: 0, b:0},
    ].reduce(&:append)
  end
